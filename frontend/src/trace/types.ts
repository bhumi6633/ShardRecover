...
export interface ChunkTrace {index:number;offset:number;type:string;length:number;crc_valid:boolean}
export interface Trace {
  ...
  format:{type:string;signature_valid:boolean;structurally_valid:boolean;width:number|null;height:number|null;valid_crc_count:number;invalid_crc_count:number;all_crc_valid:boolean;chunks:ChunkTrace[]}
